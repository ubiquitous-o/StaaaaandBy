...
There is a second failure mode: the phone's Spotify stays alive but its Connect mirror silently desyncs — it keeps reporting the last state it saw (often "paused") while the Mac plays on. Spotify only refreshes that state when its own UI comes to the foreground. So a watchdog checks every 15 s whether the session looks stale — still "playing" but the extrapolated position ran past the end of the track, or "paused" for over a minute on a session that was previously mirroring another device (detected via `AudioManager.isMusicActive()`: playing, but no audio on this phone) — and if so, launches the music app and brings the standby screen back on top 300 ms later. The app never gets to draw, but the launch alone makes it resync; the lock screen may flicker for a moment. Since a genuine long pause is indistinguishable from a desync, retries back off (immediately, then 5 / 15 / 30 min). All of this is logged under the `StaaaaandBy` logcat tag.
