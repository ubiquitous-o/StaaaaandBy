...
The mirror can also die without being torn down: the session keeps the same track but suddenly reports "paused" while the other device plays on (measured 2026-09-04, ~6 minutes into playback). That is indistinguishable from a real pause, so since v1.0.10 the watchdog's "paused for over 30 seconds on a mirrored session" verdict also switches the display to **the clock only** and disables taps. A real pause on the other device that lasts over 30 seconds looks the same, and comes back the moment playback resumes. Whenever the display is clock-only, a tap anywhere closes StaaaaandBy and returns to the normal lock screen (it comes back on the next screen-off while still charging), and since v1.0.11 the screen is dimmed to 5% while clock-only — Qi charging already pushes the battery toward the ~45°C cutoff, so the display should not add heat when there is nothing worth looking at.
